...
# 2. 相关性回归模型构建
# 提取与出口、进口相关的特征
X_export = np.column_stack((food_export_percentage, population_growth))  # 与出口相关的特征
X_import = np.column_stack((pet_count, market_size, import_volume_index, global_market_size))  # 与进口相关的特征
...
# 6. 预测未来三年中国宠物食品的生产和出口
# 预测未来三年出口值
gdp_future_features = np.column_stack((food_export_percentage[-1] * (1 + 0.05) ** np.arange(1, 4), pop_growth_rate_future))
gdp_future_features_normalized = scaler_export.transform(gdp_future_features)
pet_food_export_value_future = export_model.predict(gdp_future_features_normalized)

# 预测未来三年进口值
import_future_features = np.column_stack((pet_count_weighted_future, market_size_future, import_volume_index[-1] * (1 + 0.03) ** np.arange(1, 4), global_market_size[-1] * (1 + 0.04) ** np.arange(1, 4)))
import_future_features_normalized = scaler_import.transform(import_future_features)

# 处理可能的 inf 和 nan 值
import_future_features_normalized = np.nan_to_num(import_future_features_normalized, nan=0.0, posinf=np.finfo(np.float64).max, neginf=np.finfo(np.float64).min)

pet_food_output_value_future = import_model.predict(import_future_features_normalized)
...
# 宠物食品出口预测
plt.subplot(2, 1, 2)
plt.plot(np.concatenate((years, years_future)), np.concatenate((pet_food_export_value, pet_food_export_value_future)), '-s', label='Pet Food Export Prediction', linewidth=2, markersize=6, color='r')
...
plt.plot(np.concatenate((years, years_future)), np.concatenate((pet_food_output_value, pet_food_output_value_future)), '-^', label='Pet Food Output Prediction', linewidth=2, markersize=6, color='g')
plt.title('Pet Food Output Prediction (2019-2027)')
plt.xlabel('Year')
plt.ylabel('Output Value (Billion CNY)')
plt.legend()
plt.grid(True)
plt.show()

# 输出预测结果
print('Predicted Pet Food Industry Metrics (2025-2027):')
for i, year in enumerate(years_future):
    print(f'Year {year}:')
    print(f' - Pet Count (Million): {pet_count_weighted_future[i]:.2f}')
    print(f' - Pet Food Export Value (Billion USD): {pet_food_export_value_future[i]:.2f}')
    print(f' - Pet Food Output Value (Billion CNY): {pet_food_output_value_future[i]:.2f}')
