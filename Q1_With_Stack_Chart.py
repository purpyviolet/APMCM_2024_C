...
# 7. 使用堆叠面积图展示宠物数量变化
plt.figure(figsize=(10, 6))
plt.stackplot(years, cat_count, dog_count, total_pet_count - cat_count - dog_count, labels=['猫数量', '狗数量', '其他宠物数量'], colors=['b', 'r', 'g'])
plt.xlabel('年份')
plt.ylabel('数量（万）')
plt.title('宠物数量变化（堆叠面积图）')
plt.legend(loc='upper left')
plt.grid(True)
plt.gca().tick_params(axis='both', labelsize=12)
plt.show()
...
plt.xlabel('Year')
plt.ylabel('Number (10,000)')
plt.title('Pet Number Change (Stacked Area Chart)')
plt.legend(loc='upper left')
plt.grid(True)
plt.gca().tick_params(axis='both', labelsize=12)
plt.show()
